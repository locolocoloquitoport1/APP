 ...
 * - alerts: array de alertas { buoyId, timestamp, day, time, variable, valueRaw, deviationPct }
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ textAlign: "left", color: "#cfeffd", opacity: 0.95 }}>
                  <th style={{ padding: "6px 8px", width: 60 }}>Boya</th>
                  <th style={{ padding: "6px 8px", width: 95 }}>Día</th>
                  <th style={{ padding: "6px 8px", width: 95 }}>Hora</th>
              ...
              <tbody>
                {alerts
                  ...
                  .reverse()
                  .map((a, i) => {
                    // Preferimos fields day/time si existen (creados en App); si no, derivar desde timestamp
                    const day = a.day || (a.timestamp ? new Date(a.timestamp).toISOString().split("T")[0] : "");
                    const time = a.time || (a.timestamp ? new Date(a.timestamp).toISOString().split("T")[1].split(".")[0] : "");
